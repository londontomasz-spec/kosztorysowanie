...
function App() {
  ...
  const saveEstimate = async () => {
    ...
    try {
      ...
      console.error('Błąd zapisu kosztorysu:', error);
      alert('❌ Nie udało się zapisać kosztorysu. Sprawdź połączenie internetowe i spróbuj ponownie.');
    }
  };
  ...
  const deleteEstimate = async (estimateId) => {
    if (window.confirm('Czy na pewno chcesz usunąć ten kosztorys?')) {
      try {
        ...
        alert('❌ Nie udało się usunąć kosztorysu. Spróbuj ponownie.');
      }
    }
  };
  ...
  const handleChange = (index, field, value) => {
    ...
    // Auto-update labor price if RH/unit changes
    if (field === 'rhPerUnit') {
      newItems[index].laborPrice = Number((newItems[index][field] * hourlyRate).toFixed(2));
    }
  ...
  const addItem = async () => {
    // Sprawdź limit po stronie serwera (nie da się ominąć w DevTools)
    if (session?.user?.id) {
      try {
        const response = await fetch('/api/validate-premium', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: session.user.id })
        });

        const { isPremium, maxItems: serverMaxItems } = await response.json();

        if (items.length >= serverMaxItems) {
          alert(`Limit pozycji: ${serverMaxItems}. ${!isPremium ? 'Kup Premium aby dodać więcej!' : ''}`);
          return;
        }
      } catch (error) {
        console.error('Błąd walidacji Premium:', error);
        if (items.length >= maxItems) {
          alert('Osiągnięto limit pozycji');
          return;
        }
      }
    } else {
      if (items.length >= 3) {
        alert('Zaloguj się aby dodać więcej pozycji!');
        return;
      }
    }
  ...
  const pickService = (idx, service) => {
    ...
    if (!newItems[idx].rhPerUnit || newItems[idx].rhPerUnit === 0) {
      ...
      // Auto-calculate labor price based on global hourly rate
    ...
    <div style={{ padding: 24, minHeight: "100vh", background: theme.bg, color: theme.text, fontFamily: "sans-serif", maxWidth: "100%", width: "100%", boxSizing: "border-box", transition: "background 0.3s, color 0.3s" }}>
      ...
      <div style={{ background: theme.bgSecondary, padding: 20, borderRadius: 8, marginBottom: 24, border: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <button onClick={() => setShowTemplateDialog(true)} style={{ padding: "10px 20px", background: '#3B82F6', color: '#fff', border: "none", borderRadius: 4, cursor: 'pointer', fontSize: 14, fontWeight: 'bold' }}>
            🛁 Łazienka
          </button>
        </div>
